...
<xsl:template match="/">
 ...
 <xsl:for-each select="res:sparql/res:results/res:result">
   ...
   <xsl:for-each select="res:binding"> 
     ...
     <xsl:if test="@name='Field_Study_Investigators'">
      <div class="proj_item" >
      <strong><xsl:value-of select="translate(@name,'_',' ')"/><xsl:text>: </xsl:text></strong>
      <xsl:value-of select="res:literal"/><xsl:comment> EOD proj_item </xsl:comment></div>
     </xsl:if>
     <xsl:if test="@name!='Field_Study' and @name!='Start_Date' and @name!='End_Date' and @name!='Field_Study_URI' and @name!='Field_Study_Investigators'">
      <div class="proj_item" >
      <strong><xsl:value-of select="translate(@name,'_',' ')"/><xsl:text>: </xsl:text></strong>
      <xsl:value-of select="res:literal" disable-output-escaping="yes"/><xsl:comment> EOD proj_item </xsl:comment></div>
     </xsl:if>
  </xsl:for-each>
  <xsl:comment> EOD proj_details </xsl:comment></div>
 </xsl:for-each>
<xsl:comment> EOD proj_findings </xsl:comment></div>
</xsl:template>
