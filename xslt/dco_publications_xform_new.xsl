 ...
 <xsl:for-each select="res:sparql/res:results/res:result">
   ...
   <xsl:for-each select="res:binding"> 
     ...
     <xsl:if test="@name='DOI2'">
      ...
      <a href="http://dx.doi.org/{$doi-value-2}"><xsl:text>DOI:</xsl:text><xsl:value-of select="res:literal" disable-output-escaping="yes"/></a><xsl:text>. </xsl:text>
     </xsl:if>
     ...
     <xsl:if test="@name!='ArticleID' and @name!='Article' and @name!='Year' and @name!='Authors' and @name!='DOI1' and @name!='DOI2' and @name!='DCO_ID'">
