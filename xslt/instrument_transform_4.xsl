...
<xsl:template match="/">
 ...
 <xsl:for-each select="res:sparql/res:results/res:result">
   ...
   <xsl:for-each select="res:binding"> 
     ...
     <xsl:if test="@name='Instrument_Investigators'">
      <div class="proj_item" >
      <strong><xsl:value-of select="translate(@name,'_',' ')"/><xsl:text>: </xsl:text></strong>
      <xsl:value-of select="res:literal"/><xsl:comment> EOD proj_item </xsl:comment></div>
     </xsl:if>
     <xsl:if test="@name!='Instrument' and @name!='Start_Date' and @name!='End_Date' and @name!='Instrument_URI' and @name!='Instrument_Investigators'">
      <div class="proj_item" >
      <strong><xsl:value-of select="translate(@name,'_',' ')"/><xsl:text>: </xsl:text></strong>
